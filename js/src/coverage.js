...
define(['jquery', 'd3', 'js/src/divSelections'], function(jQuery, d3, checkDivSelection){
    return function (data, divID, title, width, height) {
        ...
        if(data && data["COV"] && data["COV"][1] && data["COV"][1].values && data["COV"][1].values.length !== 0){
          if(width && height){
            return new coverageGraph(data["COV"], divID, title, width, height);
          }else{
            return new coverageGraph(data["COV"], divID, title);
          }
        }else{
          window.console.log('data does not exist; coverage chart not created.');
          return null;
        }
    };
