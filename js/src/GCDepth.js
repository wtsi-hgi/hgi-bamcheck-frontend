...
define(['jquery', 'd3', 'js/src/divSelections'], function(jQuery, d3, checkDivSelection){
    return function (data, divID, title, width, height) {
        ...
        if(data && data["GCD"] && data["GCD"][1] && data["GCD"][1].values && data["GCD"][1].values.length > 1){
            if(width && height){
                return new gcDepthGraph(data["GCD"], divID, title, width, height);
            }else{
                return new gcDepthGraph(data["GCD"], divID, title);
            }
        }else{
