...
define(['jquery', 'd3', 'js/src/divSelections'], function(jQuery, d3, checkDivSelection){
    return function (data, divID, title, width, height) {
        ...
        if(data && data["ID"] && data["ID"][1] && data["ID"][1].values && data["ID"][1].values.length !== 0){
          if(width && height){
            return new indelDistGraph(data["ID"], divID, title, width, height);
          }else{
            return new indelDistGraph(data["ID"], divID, title);
          }
        }else{
